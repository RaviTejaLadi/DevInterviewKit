...
30. [Count reverse pairs (i<j, nums[i]>2*nums[j])](#count-reverse-pairs-(i<j-nums[i]>2*nums[j]))
...
👉 (For brevity, I’ll note: JS lacks a native heap. You’d write or use a library for efficient heaps. Below is brute-force sort way.)
...
✅ **Efficient = two heaps**. We can do that if you want to see a heap-impl JS version.
...
function maxPoints(points) {
  ...
  for (let i = 0; i < points.length; i++) {
    let map = new Map(), overlap = 0, curMax = 0;
...
function minWindow(arr, target) {
  ...
  let left=0, count=0, minLen=Infinity;
...
function maxSubArrayLen(nums, k) {
  let map = new Map([[0,-1]]), sum=0, maxLen=0;
...
function countRangeSum(nums, lower, upper) {
  ...
  function mergeSort(lo, hi) {
    ...
    let j = mid, k = mid, t = mid, cache = [], r = 0;
...
function countGood(nums,k){
  let left=0, pairs=0, map=new Map(), res=0;
  for(let right=0; right<nums.length; right++){
    let val=nums[right];
    pairs += map.get(val)||0;
...
function longestSubarray(nums, limit){
  let maxDeque=[], minDeque=[], left=0, res=0;
  for(let right=0; right<nums.length; right++){
...
function findUnsortedSubarray(nums){
  let n=nums.length,left=-1,right=-1,max=-Infinity,min=Infinity;
  for(let i=0;i<n;i++){
...
function numSubmat(mat){
  ...
  let res=0, height=Array(cols).fill(0);
...
function numberOfSubarrays(nums,k){
  let map=new Map([[0,1]]), sum=0, res=0;
  for(let num of nums){
    sum+=num%2;
    res+=(map.get(sum - k)||0);
...
function atMostK(A,K){
  let count=new Map(), res=0, left=0;
...
function maxLenEvenOdd(nums){
  let map=new Map([[0,-1]]), diff=0, res=0;
  for(let i=0;i<nums.length;i++){
    diff += nums[i]%2==0 ? 1 : -1;
    if(map.has(diff)){
...
function beautySum(s){
  ...
  for(let i=0;i<s.length;i++){
    ...
    for(let j=i;j<s.length;j++){
      ...
      let max=Math.max(...freq), min=Math.min(...freq.filter(f=>f>0));
      res+=max-min;
    }
  }
  return res;
...
function kConcatenationMaxSum(arr,k){
  ...
  let prefix=0,suffix=0,sum=0,bestPre=0,bestSuf=0;
  for(let n of arr){
    sum+=n;
    prefix+=n;
  ...
  return Math.max(maxKadane,bestPre+bestSuf+(k-2)*arr.reduce((a,b)=>a+b,0))%mod;
}
function kadane(A){
  let maxSoFar=0, maxHere=0;
  for(let n of A){
...
function countSubarrMedianK(A,K){
  let map=new Map([[0,1]]), sum=0,res=0, found=false;
  for(let n of A){
    if(n<K) sum--;
    else if(n>K) sum++;
    else found=true;
...
function reversePairs(nums){
  function mergeSort(l,r){
    if(r-l<=1) return 0;
    let m=(l+r)>>1, count=mergeSort(l,m)+mergeSort(m,r);
    let j=m;
    for(let i=l;i<m;i++){
      while(j<r && nums[i]>2*nums[j]) j++;
      count+=j-m;
    }
    nums.slice(l,r).sort((a,b)=>a-b).forEach((v,i)=>nums[l+i]=v);
    return count;
  }
...
function maximalRectangle(matrix){
  ...
  let cols=matrix[0].length, heights=Array(cols).fill(0),max=0;
...
function minSubarray(nums,p){
  ...
  if(total==0) return 0;
  let map=new Map([[0,-1]]), sum=0,min=nums.length;
  for(let i=0;i<nums.length;i++){
    sum=(sum+nums[i])%p;
...
function maxProduct(nums){
  let maxProd=nums[0], minHere=nums[0], maxHere=nums[0];

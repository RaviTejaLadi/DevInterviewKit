...
Dynamic programming — max at each point is either current element or current + previous max.
...
function findLeaders(arr) {
  let leaders = [], maxFromRight = -Infinity;
...
function countInversions(arr) {
  function mergeSort(arr) {
    ...
    let merged = [], count = left.count + right.count;
    let i=0,j=0;

...
**Explanation**: We use a hash map to store each number's index as we iterate.
For each number, we check if its complement (target - number) exists in the map.
...
**Explanation**: Convert array to Set (which removes duplicates) and compare
sizes.

### 3. Valid Palindrome

**Problem**: Check if string is palindrome after converting to lowercase and
...
**Explanation**: Brian Kernighan's algorithm - each iteration flips rightmost
1-bit.
...
**Explanation**: Use stack to track opening brackets and match with closing
ones.
